...
- 棒の高さ = 値（彩度 or 明度）、棒の色 = セルの代表色（最頻色相法。「解析」節を参照）。
...
- 見た目の既定値: 床テクスチャ ON、背景はダークグレー、ライティングは RealityKit 既定。初期カメラは仰角 45°・方位 30° でグリッド全体が収まる位置。ズームは 0.5〜4 倍に制限。明度チャートで暗い棒が見づらい点は v1 では許容し、カラーマップ切替は v2。
